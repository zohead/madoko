...
function texBibinfo(cap, context)  /* (cap : std/regex/matched, context : texParserBase/texContext) -> string */  {
  return ("[" + ($texParserBase.parse($std_regex._index_($std_regex.groups(cap), 2), context) + ("]{ .bibinfo; field:\"" + ($std_core.toLower($texParserBase.unbrace($std_regex._index_($std_regex.groups(cap), 1))) + "\"}"))));
}
...
function texDoi(cap, context)  /* (cap : std/regex/matched, context : texParserBase/texContext) -> string */  {
  ...
  var _x0 = $std_regex.find(txt, $std_regex.regex("^https?://(?:dx\\.)?doi\\.org/(.*)$", undefined, undefined), undefined);
  if (_x0 != null) {
    var path = $std_regex._index_($std_regex.groups(_x0.unJust), 1);
  }
  else {
    var path = txt;
  }
  if ((txt === "")) {
    ...
    return ("[doi:[" + (path + ("](https://doi.org/" + (urlEncode(path) + ")]{.doi}"))));
  }
}
